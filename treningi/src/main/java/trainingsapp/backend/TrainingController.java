...
class TrainingController {
    ...
    private void prepareStatements() throws BackendException {
        try {
            ...
            SELECT_TRAINING_BY_ID = session.prepare("SELECT * FROM trainings WHERE trainingId=?;");
    ...
    public Training selectTrainingById(String trainingId) throws BackendException {
        BoundStatement selectTraining = new BoundStatement(SELECT_TRAINING_BY_ID);
        ResultSet rs = null;
        try {
            selectTraining.bind(trainingId);
            rs = session.execute(selectTraining);
        } catch (Exception e) {
            throw new BackendException("Could not perform a query. " + e.getMessage() + ".", e);
        }
